...
import type { CreateSubscriptionPlanTemplateRequest, SubscriptionPlan } from "@digital-barbershop/shared-types";
...
export default function CreateSubscriptionPlanTemplate() {
    ...
    const createMutation = useMutation({
        mutationFn: (payload: CreateSubscriptionPlanTemplateRequest) =>
            subscriptionService.createSubscriptionPlanTemplate(payload),
        onSuccess: () => {
            notifySuccess("Subscription plan template created");
            queryClient.invalidateQueries({ queryKey: ["subscription-plan-templates"] });
            navigate("/dashboard/subscription-templates");
        },
        onError: (error: unknown) => {
            let message = "Failed to create plan template";
            ...
            notifyError(message, "Create template failed");
        },
    });
    ...
    const handlePlanChange = (event: React.ChangeEvent<HTMLSelectElement>) => {
        setFormValues((prev) => ({ ...prev, plan: event.target.value as SubscriptionPlan }));
    };
            ...
            <div className="flex flex-wrap items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-muted-foreground">
                        Define limits and benefits for a new subscription tier.
                    </p>
                </div>
